...
ap.add_argument('-tln','--train_layer_number', default = None, required = False, type = str, nargs='+', help= 'which layer is going to be trained, ex -ltn 9 10 11')
ap.add_argument('--not_lower', action='store_false', help="Set this flag if you are using an cased model.")
...
train_logger = Logger(args['save_path'] + '/train.log')

tok = BertTokenizer(args['vocab'], do_lower_case=args['not_lower'])
    ...
    for epoch in range(num_epoc):
        ...
        for batch_ind in range(int(len(train_data_tok)/batch_size)):
            ...
            if batch_ind>0 and batch_ind % 10 ==0:
                ...
                train_logger.write([epoch, batch_ind*batch_size, train_loss.item()/10/batch_size, val_loss.item()/10/batch_size, train_acc.item()/10, val_acc.item()/10, train_f1/10, val_f1/10])
                train_loss = 0
                val_loss = 0
                train_acc = 0
                val_acc = 0          
...
best_state_dict = t.load(args['save_path']+'/stat_dic_'+args['strategy']+'_2', map_location = 'cpu')
...
with t.no_grad():
    ...
    test_logger.write([f1_score(LABEL, MODEL_OUT, average = 'macro')])
