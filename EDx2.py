...
def main(rank, args):
    ...
    tokenizer = BertTokenizer.from_pretrained('/nas/home/tmddnjs3467/NLP/exBERT/pretrained/5_3G/only_ex_with_vocab_256', do_lower_case=args.do_lower_case)
    tokenizer_bert = BertTokenizer.from_pretrained(args.teacher_model, do_lower_case=args.do_lower_case)
    ...
    from exBERT.modeling import BertModel, BertForPreTraining, BertConfig, TinyBertForPreTrainingWithDistillation
    bert_config = BertConfig.from_json_file('/nas/home/tmddnjs3467/NLP/exBERT/bert_base/uncased/config.json')
    print("Building PyTorch model from configuration: {}".format(str(bert_config)))
    teacher_model = BertModel(bert_config)
    stat_dict = torch.load('/nas/home/tmddnjs3467/NLP/exBERT/bert_base/uncased/097417381d6c7230bd9e3557456d726de6e83245ec8b24f529f60198a67b203a', map_location='cpu')
    ...
    tiny_bert_config_1 = BertConfig.from_json_file('./config_for_exbert1.json')
    tiny_bert_config_2 = BertConfig.from_json_file('./config_for_exbert1_33.json')
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        epoch_dataset_bert = PregeneratedDataset(epoch=epoch, training_path=args.pregenerated_data, tokenizer=tokenizer_bert,
                                            num_data_epochs=num_data_epochs, reduce_memory=args.reduce_memory)
        if args.local_rank == -1:
            train_sampler = RandomSampler(epoch_dataset)
        else:
            train_sampler = DistributedSampler(epoch_dataset, rank=args.local_rank, num_replicas=args.world_size)
            train_sampler_bert = DistributedSampler(epoch_dataset_bert, rank=args.local_rank, num_replicas=args.world_size)
        train_dataloader = DataLoader(epoch_dataset, sampler=train_sampler, batch_size=args.train_batch_size)
        train_dataloader_bert = DataLoader(epoch_dataset_bert, sampler=train_sampler_bert, batch_size=args.train_batch_size)
        ...
        with tqdm(total=len(train_dataloader), desc="Epoch {}".format(epoch)) as pbar:
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                batch_bert = tuple(t.to(device) for t in next(bert_iter))

                input_ids, input_mask, segment_ids, lm_label_ids, is_next = batch
                input_ids2, input_mask2, segment_ids2, lm_label_ids2, is_next2 = batch_bert
                ...
                ce_loss, student_atts = student_model(input_ids, segment_ids, input_mask, lm_label_ids, is_next)
                _, teacher_atts, _ = teacher_model(input_ids2, segment_ids2, input_mask2)
                ...
                for student_att, teacher_att in zip(student_atts, new_teacher_atts):
                    ...
                    teacher_att = torch.where(teacher_att <= -1e2, torch.zeros_like(teacher_att).to(device),
                                              teacher_att)
                    att_loss += loss_mse(student_att[:, :12], teacher_att)
                ...
                loss = att_loss + ce_loss
                if n_gpu > 1:
                    # loss = loss.mean()  # mean() to average on multi-gpu.
                    loss = reduce_tensor(loss.data, dist.get_world_size)
                    att_loss = reduce_tensor(att_loss.data, dist.get_world_size)
                    # rep_loss = reduce_tensor(rep_loss.data, dist.get_world_size)
                    ce_loss = reduce_tensor(ce_loss.data, dist.get_world_size)
                if args.gradient_accumulation_steps > 1:
                ...
                mean_ce_loss = tr_ce_loss * args.gradient_accumulation_steps / nb_tr_steps
